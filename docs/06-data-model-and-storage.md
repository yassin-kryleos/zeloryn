...
- Persisted metadata must include schema version where migrations may apply.
- JSON metadata writes must use atomic write, flush, rename, and safe-lock helpers rather than direct overwrite.
...
type CriterionPriority = 'required' | 'optional';
type CriterionFlakeStatus = 'stable' | 'suspected_flaky' | 'known_flaky';
...
interface AcceptanceCriterion {
  ...
  confidence?: number;
  timeoutMs?: number;
  flakeStatus?: CriterionFlakeStatus;
...
interface SkillDefinition {
  ...
  content: string;
  permissions?: Array<'read' | 'write' | 'execute' | 'network' | 'admin'>;
...
- Never partially rewrite JSON state in place.
- Use JSONL append for audit-style records only when replay can tolerate the last partial line being ignored.

Storage lock rules:

- Lock records include PID, hostname, process start token, created timestamp, and app version.
- Stale locks are cleared only when PID, hostname, and start token prove the owner is gone.
- If ownership is ambiguous, fail closed and show a recovery diagnostic instead of risking corruption.
- Migration runners create a backup before mutation and reject unknown future schema versions unless a read-compatible path exists.
