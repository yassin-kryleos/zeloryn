...
KRYLEOS_BACKEND_PORT=3001
KRYLEOS_BACKEND_PORT_FALLBACK=random_loopback
...
KRYLEOS_LOG_LEVEL=info
KRYLEOS_LOG_REDACTION=true
...
- If `3001` is unavailable, Electron main may choose a random loopback port and broker it to the renderer.
...
- Health endpoint returns status/version only.
- Backend supervisor readiness and restart behavior works.
- Renderer receives active backend port from Electron main.
- Backend binds admin API to loopback.
- Companion channel binds to loopback unless explicitly configured.
- Electron runs with `contextIsolation`, `nodeIntegration:false`, restrictive production CSP, IPC validation, blocked untrusted navigation, and gated external URL opening.
- App can select workspace.
- App can run a smoke FORGE task with approval.
- `.kryleos` storage uses schema version, atomic writes, and lock files.
...
- Structured logs are local, rotated, and redacted by default.
...
- Startup/backend supervisor log summary if safe.
- Error message.
...
- Startup.
- Backend readiness, active port, restarts, and supervisor stop reason.
- Storage lock decisions and migration status without private file contents.
- Workspace selection.
